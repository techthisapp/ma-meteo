...
/* ---------- Les planètes ----------
   ...
   Les cinq que l'œil nu distingue, par leurs éléments d'orbite rapportés à
   l'époque J2000 et leur dérive par siècle. L'orbite est réduite à une ellipse
   képlérienne, sans les perturbations mutuelles des planètes : la position s'en
   écarte de quelques minutes d'arc au plus sur les décennies qui nous entourent,
   ce qui ne se voit pas sur une carte du ciel où la plus petite étoile occupe
   déjà un demi-degré de doigt. Éléments de la NASA, « Keplerian elements for
   approximate positions of the major planets ».

   Chaque ligne donne, dans l'ordre : le demi-grand axe en unités
   astronomiques, l'excentricité, l'inclinaison, la longitude moyenne, la
   longitude du périhélie, la longitude du nœud ascendant, puis leurs dérives
   par siècle. */
const PLANETES = {
  mercure: { nom: "Mercure",
...
export const NOMS_PLANETES = Object.fromEntries(
  Object.entries(PLANETES).map(([c, p]) => [c, p.nom]));

/* Les coordonnées héliocentriques d'une planète, en unités astronomiques, dans
   le plan de l'écliptique de J2000. L'équation de Kepler se résout par
   itérations successives, qui convergent en quelques tours pour des
   excentricités aussi faibles. */
export function heliocentrique(cle, jj) {
  const p = PLANETES[cle];
  const T = (jj - 2451545) / 36525;
  const [a, e, I, L, pi, O] = p.e0.map((v, k) => v + p.d[k] * T);
  const w = pi - O;
  let M = tour(L - pi);
  if (M > 180) M -= 360;
  let E = M + (180 / Math.PI) * e * sin(M);
  for (let k = 0; k < 8; k++) {
    const dM = M - (E - (180 / Math.PI) * e * sin(E));
    E += dM / (1 - e * cos(E));
  }
  const xp = a * (cos(E) - e);
...
/* La planète vue de la Terre : sa position héliocentrique moins celle de la
   Terre, puis le passage aux coordonnées équatoriales. */
export function planete(cle, jj) {
  const p = heliocentrique(cle, jj);
  const t = heliocentrique("terre", jj);
  const x = p.x - t.x, y = p.y - t.y, z = p.z - t.z;
  const lambda = tour(Math.atan2(y, x) * 180 / Math.PI);
  const beta = Math.atan2(z, Math.hypot(x, y)) * 180 / Math.PI;
  const eps = obliquite(jj);
  const ascension = tour(Math.atan2(
    sin(lambda) * cos(eps) - Math.tan(beta * Math.PI / 180) * sin(eps), cos(lambda)) * 180 / Math.PI);
  const declinaison = Math.asin(
    sin(beta) * cos(eps) + cos(beta) * sin(eps) * sin(lambda)) * 180 / Math.PI;
  return { lambda, beta, ascension, declinaison, distance: Math.hypot(x, y, z) };
}

/* Position d'un corps pour un instant donné en temps universel. */
const positionDe = (corps, jj) =>
  (corps === "lune" ? lune(enTT(jj))
    : PLANETES[corps] ? planete(corps, enTT(jj))
    : soleil(enTT(jj)));
